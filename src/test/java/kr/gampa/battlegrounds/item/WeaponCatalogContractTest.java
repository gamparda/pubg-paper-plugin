...
class WeaponCatalogContractTest {
  ...
  @Test void catalogContainsAllAttachedWeaponsWithValidMaterialsAndSources(){
    ...
    for(String id:weapons.getKeys(false)){String path="weapons."+id+".";assertNotNull(Material.matchMaterial(c.getString(path+"material")),id);assertFalse(Set.of("SPYGLASS","BRUSH").contains(c.getString(path+"material")),id+" must not trigger a vanilla use animation");assertTrue(c.getString(path+"source-url","").startsWith("https://namu.wiki/w/"),id);assertTrue(c.getDouble(path+"base-damage")>=0,id);assertTrue(c.getInt(path+"cooldown-ticks")>0,id);}
  }
