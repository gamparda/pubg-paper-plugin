...
public final class GameplayListener implements Listener {
  private final BattlegroundsPlugin plugin; private final ItemRegistry items; private final PlayerSystemManager systems; private final DamageCalculator calculator=new DamageCalculator(); private final Map<UUID,FireControl> fireControls=new HashMap<>(); private final Map<UUID,String> controlledWeapon=new HashMap<>(); private final Map<UUID,Integer> controlledSlot=new HashMap<>();private int activeBullets;
  ...
  private void shootOnce(Player shooter,ItemStack gun,String id){
    ...
    muzzleParticles(shooter);int pellets=c.getInt("pellets",1),tracerPellets=Math.max(1,plugin.getConfig().getInt("ballistics.shotgun-tracer-pellets",3));List<ShotOffset> pattern=ShotPattern.create(pellets,c.getDouble("spread-degrees",0),System.nanoTime());WeaponStats weapon=weapon(id,c);int pellet=0;
    for(ShotOffset offset:pattern){org.bukkit.util.Vector direction=shooter.getEyeLocation().getDirection().clone().rotateAroundY(Math.toRadians(offset.yawDegrees()));org.bukkit.util.Vector right=direction.clone().crossProduct(new org.bukkit.util.Vector(0,1,0)).normalize();direction.rotateAroundAxis(right,Math.toRadians(offset.pitchDegrees()));if(melee)resolveHit(shooter,direction,c,weapon);else launchBallistic(shooter,direction.normalize(),c,weapon,pellet++<tracerPellets);}
  ...
  private void launchBallistic(Player shooter,org.bukkit.util.Vector direction,ConfigurationSection c,WeaponStats weapon,boolean showTrail){
    if(!plugin.getConfig().getBoolean("ballistics.enabled",true)){resolveHit(shooter,direction,c,weapon);return;}int max=plugin.getConfig().getInt("ballistics.max-active-bullets",500);if(activeBullets>=max){resolveHit(shooter,direction,c,weapon);return;}activeBullets++;double speed=Math.max(1,c.getDouble("bullet-speed-mps",500)),gravity=plugin.getConfig().getDouble("ballistics.gravity-mps2",9.81)*c.getDouble("gravity-scale",1),drag=c.getDouble("drag-per-second",plugin.getConfig().getDouble("ballistics.drag-per-second",.01)),range=c.getDouble("max-range"),spacing=Math.max(.25,plugin.getConfig().getDouble("ballistics.trail-spacing-blocks",2)),markerSpacing=Math.max(1,plugin.getConfig().getDouble("ballistics.drop-marker-spacing-blocks",25)),raySize=plugin.getConfig().getDouble("ballistics.ray-size",.18);Location start=shooter.getEyeLocation().add(direction.clone().multiply(.35));World bulletWorld=start.getWorld();BallisticVector initialPos=new BallisticVector(start.getX(),start.getY(),start.getZ()),initialVelocity=new BallisticVector(direction.getX()*speed,direction.getY()*speed,direction.getZ()*speed);Color trailColor=Color.fromRGB(plugin.getConfig().getInt("ballistics.trail-color.r",255),plugin.getConfig().getInt("ballistics.trail-color.g",210),plugin.getConfig().getInt("ballistics.trail-color.b",80));Particle.DustOptions trailDust=new Particle.DustOptions(trailColor,(float)plugin.getConfig().getDouble("ballistics.trail-size",.65)),markerDust=new Particle.DustOptions(Color.fromRGB(255,70,30),1.15f);
    new BukkitRunnable(){BulletTrajectory trajectory=new BulletTrajectory(initialPos,initialVelocity,gravity,drag);double travelled,nextMarker=markerSpacing;boolean done;private void finish(){if(done)return;done=true;activeBullets--;cancel();}public void run(){try{if(!shooter.isOnline()||shooter.getWorld()!=bulletWorld||trajectory.position().y()<bulletWorld.getMinHeight()-8||travelled>=range){finish();return;}BulletTrajectory next=trajectory.step(.05);BallisticVector from=trajectory.position(),to=next.position();boolean finalSegment=from.distance(to)>=range-travelled;to=BulletTrajectory.clipSegment(from,to,Math.max(0,range-travelled));org.bukkit.util.Vector delta=new org.bukkit.util.Vector(to.x()-from.x(),to.y()-from.y(),to.z()-from.z());double segment=delta.length();if(segment<=0){finish();return;}Location fromLoc=new Location(bulletWorld,from.x(),from.y(),from.z());RayTraceResult hit=bulletWorld.rayTrace(fromLoc,delta.clone().normalize(),segment,FluidCollisionMode.NEVER,true,raySize,e->e instanceof LivingEntity&&e!=shooter);BallisticVector visibleTo=to;if(hit!=null)visibleTo=new BallisticVector(hit.getHitPosition().getX(),hit.getHitPosition().getY(),hit.getHitPosition().getZ());double visibleLength=from.distance(visibleTo);if(showTrail)for(BallisticVector p:BulletTrajectory.sampleSegment(from,visibleTo,spacing))bulletWorld.spawnParticle(Particle.DUST,p.x(),p.y(),p.z(),1,0,0,0,0,trailDust);while(showTrail&&nextMarker<=travelled+visibleLength){double t=(nextMarker-travelled)/Math.max(.001,visibleLength);BallisticVector p=from.lerp(visibleTo,Math.min(1,t));bulletWorld.spawnParticle(Particle.DUST,p.x(),p.y(),p.z(),1,0,0,0,0,markerDust);nextMarker+=markerSpacing;}if(hit!=null){double totalDistance=travelled+visibleLength;if(hit.getHitEntity() instanceof LivingEntity target)applyHit(shooter,target,hit.getHitPosition(),totalDistance,weapon);else bulletWorld.spawnParticle(Particle.SMOKE,hit.getHitPosition().getX(),hit.getHitPosition().getY(),hit.getHitPosition().getZ(),4,.04,.04,.04,.01);finish();return;}if(finalSegment){finish();return;}travelled+=segment;trajectory=next;}catch(RuntimeException error){plugin.getLogger().warning("탄도 작업 오류: "+error.getMessage());finish();}}}.runTaskTimer(plugin,0,1);
  }

  private void muzzleParticles(Player shooter){Location muzzle=shooter.getEyeLocation().add(shooter.getEyeLocation().getDirection().multiply(.55));shooter.getWorld().spawnParticle(Particle.FLASH,muzzle,1);shooter.getWorld().spawnParticle(Particle.SMOKE,muzzle,3,.035,.035,.035,.01);}

  private void applyHit(Player shooter,LivingEntity target,org.bukkit.util.Vector hitPosition,double distance,WeaponStats weapon){
    if(target instanceof Player victim&&!plugin.getConfig().getBoolean("combat.friendly-fire")&&sameTeam(shooter,victim))return;double ratio=(hitPosition.getY()-target.getBoundingBox().getMinY())/Math.max(.01,target.getBoundingBox().getHeight());HitLocation hit=ratio>=plugin.getConfig().getDouble("combat.head-threshold",.72)?HitLocation.HEAD:(ratio<.35?HitLocation.LIMB:HitLocation.BODY);if(target instanceof Player victim&&ratio<.55&&hasItem(victim,"frying-pan")&&victim.getLocation().getDirection().setY(0).dot(shooter.getLocation().toVector().subtract(victim.getLocation().toVector()).setY(0).normalize())<-.3){victim.getWorld().playSound(victim,Sound.BLOCK_ANVIL_LAND,1f,1.8f);return;}ItemStack armorItem=armorFor(target,hit);DamageResult damage=calculator.calculate(weapon,hit,distance,armorStats(armorItem));double before=target.getHealth();target.setNoDamageTicks(0);target.damage(damage.healthDamage()/plugin.getConfig().getDouble("combat.health-scale",5),shooter);if(target.getHealth()<before)damageArmor(armorItem,damage.armorDamage());target.getWorld().spawnParticle(Particle.CRIT,hitPosition.getX(),hitPosition.getY(),hitPosition.getZ(),5,.1,.1,.1,.1);if(hit==HitLocation.HEAD)shooter.sendActionBar(Component.text("헤드샷! 피해 "+Math.round(damage.healthDamage())));
  }
